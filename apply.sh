...
original_author="litoxy"
original_name="pov_sys_dp"
original_urlname="pov_sys_dp"
original_description="Awesome pov_sys_dp created by litoxy" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/pov_sys_dp "${name}"
...
then
    ...
    echo "then run 'pov_sys_dp' or 'python -m pov_sys_dp'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'pov_sys_dp' or 'python -m pov_sys_dp' or 'poetry run pov_sys_dp'"
fi
